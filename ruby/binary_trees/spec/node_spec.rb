...
describe Node do
  ...
  context "direct descendant behavior" do
	...
	it "knows whow its parent it" do
	  expect(node2.parent).to eq subject
	end
      end
    end
  ...
  describe "it sort one node" do
    subject { Node.new(10, true) }
    it "should be sort itself" do
      expect(subject.sort).to eq [10]
    end
  end

  describe "it can sort two number" do
    let(:leaves) { [8] }
    subject { Node.new(10, true) }
    before :each do
      leaves.each { |leaf| subject.branch leaf }
    end

    it "should sort itself out" do
      answer = subject.sort
      expect(answer).to eq [8,10]
    end
  end
  ...
  describe "it can sort two number" do
    ...
    it "should sort itself out" do
      answer = subject.sort
      expect(answer).to eq [8,10,11]
    end
  end

  describe "it can sort 5 number" do
    let(:leaves) { [8,11, 12, 3] }
    subject { Node.new(10, true) }
    before :each do
      leaves.each { |leaf| subject.branch leaf }
    end
    ...
    it "should sort itself out" do
      answer = subject.sort
      expect(answer).to eq [3,8, 10, 11, 12]
    end
  end

  describe "it can sort 11 number" do
    let(:leaves) do
      nums = (1..11).to_a
      nums.delete(10)
      return nums.shuffle
    ...
    it "should sort itself out" do
      answer = subject.sort
      expect(answer).to eq (1..11).to_a.sort
    end
  end
end
